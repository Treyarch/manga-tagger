...
A CBR list uses `lsar` and does not extract the archive. A CBR single-member read uses `unar` to extract that member into a temporary directory, then deletes the temporary directory. A full extract is reserved for convert and for saving metadata on a `.cbr`.
...
`write_number` controls `Number` only. When it is false, a `Number` entry in the patch is ignored and the element already in the file stays. When it is true, `Number` is applied like any other patched field. One selected volume uses `write_number` true. `Volume` follows the patch rules above and is never derived from `Number`. A multi-volume save does not call this with `Number` or `Volume`. It uses the batch save below.
...
`Title`, `Number`, `Volume`, `Summary`, `PageCount`, `Pages`, dates, `Web`, `Notes`, `AgeRating`, `Manga`, and `CommunityRating` are not batch fields. If `patch` contains any other key, `save_many` raises `BatchFieldError` before it writes any file.
...
The template is a filename stem. `{Series} v{Volume:02}` is the template the action offers. On a `.cbz` whose `Series` is `Claymore` and whose `Volume` is `1`, the new name is `Claymore v01.cbz`. The existing extension is kept, including its letter case. A template that itself ends in `.cbz` or `.cbr` is rejected with `RenameTemplateError` before any file is renamed. The extension is not part of the pattern.

A tag is `{` + an owned element name + `}`, optionally followed by `:` and a positive decimal width. Names match the model (`Series`, `Volume`, `Number`, and the other owned elements except `Pages`). `{Volume}` reads the `Volume` element. `{Number}` reads `Number`. The module does not substitute one for the other. `{Volume:02}` zero-pads an integer value to at least that many digits, so `1` becomes `01` and `100` stays `100`. A value that is not an integer fails that file with `RenameFieldError`. An unknown tag, a width that is not a positive integer, an unclosed `{`, or a template whose stem is empty raises `RenameTemplateError` and renames nothing.

When `{stem}-poster.jpg` already sits beside the archive, the rename moves that poster to the new stem in the same directory. A missing poster does not fail the rename. A poster name collision uses `RenameConflictError` and leaves both the archive and the poster in place.
...
| `RenameFieldError` | A file’s ComicInfo has no text for a tag the template uses, or the rendered stem is empty |
...
The rename action takes the template from the caller. The template it offers is `{Series} v{Volume:02}`. That string is not a TOML key.

Poster width is 600 pixels and JPEG quality is 85. Neither value is a TOML key. Pillow is the library that scales the cover.
...
- A metadata save copies image bytes, compression method, CRC, uncompressed size, and member names unchanged, including a deflated image and a stored image in the same archive. Unknown elements, unknown attributes, and `Pages` are still present and unchanged. The test does not decode image pixels.
...
- `save_many` with `Series` and `Writer` updates those elements on each selected file and leaves `Title`, `Number`, and `Volume` as they were. A patch that also contains `Title` or `Number` raises `BatchFieldError` and writes neither file.
...
- `rename_in_directory` with `{Series} v{Volume:02}` renames `old.cbz` to `Claymore v01.cbz` when `Series` is `Claymore` and `Volume` is `1`, and the zip bytes are unchanged. An existing `old-poster.jpg` becomes `Claymore v01-poster.jpg`. A sibling with a blank `Volume` stays at its old name. An archive in a subdirectory is not renamed. Two files that render to the same name both stay put. A template of `{Series} v{Volume:02}.cbz` or `{NotAField}` raises `RenameTemplateError` and renames nothing.
- `write_poster` reads only the cover page, writes a JPEG 600 pixels wide beside the archive, and leaves the archive bytes unchanged. A cover narrower than 600 pixels is not enlarged.
...
- A metadata save replaces `ComicInfo.xml` and copies every other member unchanged, including compression method and page bytes. `Pages` is not rebuilt. Unknown XML is still there after the save.
...
- A batch save writes only `Series`, `Publisher`, `LanguageISO`, `Genre`, `Writer`, `Penciller`, `Inker`, and `CoverArtist`. Any other patch key writes nothing. A failed file does not roll back files already written.
- Rename uses each file’s own ComicInfo, keeps the file in the same directory, keeps its extension, and does not change archive bytes. The offered template `{Series} v{Volume:02}` produces `Claymore v01.cbz` from `Volume` `1`. A blank tag or a name collision skips that file. An invalid template renames nothing. An existing sibling poster moves with the archive.
- `write_poster` writes `{stem}-poster.jpg` from the cover only, 600 pixels wide, without modifying the archive.
...
- A missing `unar` or `lsar` fails CBR operations with an error that names `unar`, and does not affect CBZ operations.
