...
Actions sit at the trailing edge: a `quiet` dismiss button labeled `Cancel`. When the dialog has a confirm action, a `primary` or `danger` confirm button follows. `MatchesDialog.svelte` uses Cancel plus a primary `OK` that confirms the highlighted candidate. While a Search is queued or running, the body shows muted `Searching…` with a spinning Lucide `LoaderCircle` (20px) and `aria-busy`. When candidates arrive, that body is replaced by a two-column picker: a large cover preview on the left (2:3 frame, `object-contain`, Lucide `Book` placeholder on miss or load failure) and a right stack with a Series / Year / Issues / (Publisher or Author) table above a muted summary pane. The credit column caption is `Publisher` when the selected provider is Comic Vine, and `Author` for MangaDex, AniList, and Jikan. The first candidate is highlighted. A single click or arrow key changes the highlight and updates the cover and summary; it does not load. OK or a double-click starts load for the highlighted id. OK is disabled while searching, while a Load is busy, or when nothing is highlighted. Row selection uses the existing accent wash (`blue-600/10` / dark `blue-500/15`), not a solid fill. MangaDex covers (`uploads.mangadex.org`) load through `GET /api/cover?url=` so the image is same-origin; other hosts use the candidate URL directly. The preview `img` uses `referrerpolicy="no-referrer"`. Match rows are not shown in the inspector. In-progress scrape status is not a toast.
...
- Dialogs are centered over the window with a dimmed backdrop. Their panels use the same raised surface as toasts (white / `zinc-800` in dark, hairline, shadow) and fade in with a short upward overshoot. The title is bold with the opening control's Lucide icon ahead of it, and a Close `X` at the trailing edge of the title row. Settings, Rename, Convert, and Matches share that chrome. Matches uses the `xl` panel width; while searching it shows `Searching…` with a spinner, then a large cover beside a Series/Year/Issues/(Publisher or Author) table and a summary pane, with Cancel plus OK. The other three keep `md` width, Cancel, and a confirm action.
