...
`PUT` replaces `library_roots` wholesale. Omitted keys stay. When the body includes `library_roots`, a scan is enqueued after the successful write, including when the stored list is empty. Other keys do not enqueue a scan.
...
`SHARED_FIELDS` is `Series`, `Publisher`, `LanguageISO`, `Genre`, `Writer`, `Penciller`, `Inker`, `CoverArtist`.
...
`form_from_volumes(rows)` returns no form when `rows` is empty. One row returns `mode` `one` and a `values` object with every `FORM_FIELDS` key. Several rows return `mode` `many` and only `SHARED_FIELDS`. Each shared field is `{ "value", "mixed", "dirty" }`. `dirty` starts false. When every selected row has the same text, `value` is that text and `mixed` is false. When they differ, `value` is `""` and `mixed` is true. Editing a field sets `dirty` to true and `mixed` to false, and `value` becomes what the user typed, including `""` when the user clears it.
...
Several volumes: the inspector shows only the shared fields. A mixed field's placeholder is `Mixed`. A field that is not `dirty` is omitted from the save patch, whether it is mixed or the same on every row. An empty uniform value is not sent as `""`, so Save does not remove an element the user did not edit.

The form is rebuilt from the index rows when the selection changes and when a save, rename, or convert refetch completes.
...
`JobRunner` runs one job at a time, in start order. It does not use a process pool. A job has `id`, `name`, `state`, `error_type`, `error_message`, and `result`. Ids are decimal strings starting at `1`. States are `queued`, `running`, `succeeded`, `failed`, and `cancelled`. `error_type` and `error_message` are `""` unless the state is `failed`. `result` is null unless the job succeeded or was cancelled with a partial result.

`start(name, fn)` queues `fn`. The function receives `cancel`, a callable that returns true after a cancel request. `inline=True` runs the job to a terminal state on the caller thread before `start` returns. Tests use the inline runner. They do not call `time.sleep`.

`cancel(id)` on a queued job sets `cancelled` and does not call `fn`. On a running job it sets the flag and does not abort the thread. A provider request that has already started is not aborted. `get` of an unknown id raises `JobNotFoundError`. `current` is the most recently started job, or null when none has been started.
...
Header text for a `queued` or `running` job is `Scan`, `Search`, `Load`, `Save`, `Rename`, or `Convert`. The Cancel control is shown only then. Its accessible name is `Cancel`.
...
| `GET /api/jobs/current` | The most recent job, or JSON null |
...
`load` calls the provider `load` with the anchor `filename_stem` and the same config and client rules. The job then runs `merge_load_patch(form, patch, mode)`. For `one`, each patch key that is in `FORM_FIELDS` replaces that form value. Keys the patch omits stay. For `many`, each patch key that is in `SHARED_FIELDS` sets that field's `value`, sets `mixed` to false, and sets `dirty` to true. Every other patch key is ignored, including `Title`, `Number`, `Summary`, `Manga`, `Web`, and dates. The result is `{ "form": { "mode", "values" } }`. The client replaces its form with that object when the selection is still the one it sent. No archive is written.
...
Save does nothing when the selection is empty. The patch for one volume is every `FORM_FIELDS` value on the form, excluding `Pages`. `mode` is `one` and `paths` has that one path. `write_number` is true.

The patch for several volumes is the shared fields whose `dirty` is true, including a field the user cleared to `""` and a field a load just set. Fields that are still not `dirty` are absent, not `""`. `mode` is `many`. `write_number` is false. An empty patch does not call `save_comic_info`. The job succeeds with `{ "entries": [] }`.

Before any write, a `one` save whose `paths` length is not 1, or whose patch contains a key outside `FORM_FIELDS`, raises `BatchFieldError` for a bad key and `ShellError` for the path count. A `many` save whose patch contains a key outside `SHARED_FIELDS` raises `BatchFieldError`. `Pages` is never a patch key. These failures write nothing and do not enqueue a job when the route can see them. The job repeats the key check so a direct call is just as strict.

The job then calls `save_comic_info(path, patch, write_number=..., keep_cbr_original=...)` once per path. It does not call `save_many`, because that function cannot check cancel between files. The allowed keys, `write_number` false, and continue-on-error are the batch-save rules in the archives specification. `keep_cbr_original` is the config value copied when the job starts.

Cancel is checked before each file. A true flag raises `JobCancelled` with the entries so far. The current file is not interrupted. A file that raises records `{ "path", "error_type", "error_message" }` and the loop continues. A success records `{ "path", "output_path" }`. `output_path` is the `.cbz` path when a `.cbr` save converted it, otherwise the same path. After each success the job calls `refresh_volume` on `output_path` and, when `output_path` differs, `forget_volume` on the old path. Save does not call `write_poster`.
...
Rename is enabled when a place is selected, including when the volume selection is empty. It renames every archive directly in that place, not only the selection. The dialog's text input starts as `{Series} v{Volume:02}`. That string is not written to config. Dismiss writes nothing.
...
Convert is enabled when at least one selected file has extension `cbr`, compared case-insensitively. Selected `.cbz` files are entries `{ "path", "skipped": true }`. They are not errors and are not passed to `convert_cbr`. Each `.cbr` is `convert_cbr(path, keep_cbr_original=...)`. Cancel, per-file failure, `refresh_volume`, and `forget_volume` follow the save loop. Convert does not call `write_poster`.
...
It is the only reader and writer of `library_roots`, `keep_cbr_original`, `comicvine_api_key`, and `title_languages` from [00-project-overview.md](00-project-overview.md), and of `theme` from [05-ui-design.md](05-ui-design.md). Defaults stay those documents' defaults. View mode, the selected place, the provider choice, and the rename template are not keys. The HTTP port is not a key. The `httpx` timeout of 15 seconds is not a key.
...
- `form_from_volumes` on one row includes `Number` and omits `Pages`. On two rows it includes only `SHARED_FIELDS`, marks a differing `Series` mixed, shows a shared `Publisher`, and sets every `dirty` flag false.
- `merge_load_patch` for `one` applies `Number` and leaves a form field the patch omits. For `many` it applies `Series`, sets that field `dirty`, and ignores `Number`, `Title`, and `Manga`.
- `GET /api/library` calls `list_volumes` and does not call `scan`.
- A `one` save calls `save_comic_info` with `write_number` true and the form's `Number`. A `many` save calls it once per path with `write_number` false and without `Number`. A shared `Series` the user did not edit is absent from that patch. A `many` patch that contains `Title` raises `BatchFieldError` and does not call `save_comic_info`. An empty `many` patch does not call `save_comic_info`.
...
- A rename job calls `rename_in_directory` on the place and calls `write_poster` on each success path. A convert job skips a `.cbz` and calls `convert_cbr` for a `.cbr`.
...
- Cancel of a queued job does not call its function. The inline runner does not sleep.
...
- A place is a directory that directly contains indexed volumes. The main pane lists those volumes only. Nested volumes are a different place. The header field filters `series`, `title`, and filename, and does not scrape.
- One selected volume shows that volume's form and one preview page. Several selected volumes show one shared form for `Series`, `Publisher`, `LanguageISO`, `Genre`, `Writer`, `Penciller`, `Inker`, and `CoverArtist`. A shared field is written only after the user edits it or a load sets it. The preview follows the anchor.
- Search and load fill the form and do not write an archive. One volume can take `Number` from the filename stem. Several volumes do not. A blank Comic Vine key fails before a request.
- Save writes through `save_comic_info`. One volume may write `Number`. Several volumes write only the shared fields. Cancel stops before the next file. A failed file does not stop the rest, and files already written stay written. Save does not write the sibling poster.
- Rename runs on the selected place with the offered template `{Series} v{Volume:02}`, then `write_poster` on each success. Convert turns selected `.cbr` files into `.cbz` and skips `.cbz`. A path outside the library is rejected before any read or write.
- Scrapes, saves, converts, and rescans are jobs on one worker. The user can cancel. Closing the window asks the worker to stop and does not leave a truncated archive from this process killing a write.
...
- **How does the UI hear about background work?** It polls the job. Tests use an inline runner and do not sleep.
