...
- Search and load fill the form and do not write an archive. Scrape opens the centered Matches dialog in a searching state; results replace that body, dismiss while searching cancels the job, an empty or failed search closes the dialog, and a successful load clears the list so the dialog closes. A load marks the fields it sets dirty, including `Manga` on a shared form. A blank Comic Vine key fails before a request.
