...
Each archive is one tankōbon. The volume number is stored in ComicInfo `Number` and mirrored into `Volume` whenever `Number` is written. A scrape load fills `Count` with the catalog's released volume or issue count when that count is known.
